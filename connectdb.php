...
//	 echo $rows["pdf"] . $rows["url"];
    ...
    if(isset($_POST['$pdf'])) {
		...
		else if($rows["address"] != "Jaipur")
			echo "No constituency match";
     else {
		echo 'Candidate Name:' ." " . $rows["candidate_name"] . "<br>" . "<br>" .  'Age:' . " " . $rows["age"] . "<br>" . "<br>" . 'Address:' . " " . $rows["address"] . "<br>" . "<br>" .  'url:' . " " .  "<td><a href='" . $url . "'>" . $url . "</a></td>" . "<br>" . "<br>" .  'pdf:' . " " . "<td><a href='" . $pdf . "'>" . $pdf . "</a></td>";
      }
}
   ...
   if(isset($_POST['$pdf'])) {
     ...
     if(isset($_POST['$pdf'])) {
		...
		$sql = mysqli_query($con, "SELECT * FROM candidate WHERE candidate_name = 'Chand Singh' ");
		$rows = mysqli_fetch_assoc($sql);
		$url = $rows["url"];
		$pdf = $rows["pdf"];
		if(mysqli_num_rows($sql) == 0) {
   	...
   	//	if($rows["candidate_name"] != "Chand Nath" && $rows["age"] != "61" && $rows["address"] != "Alwar")
   		//		echo "Name, age and address of the candidate don't match!";
...
//	 echo $rows["pdf"] . $rows["url"];
		...
		//	echo "<strong>" . "Candidate and Constituency match!" . "</strong>";
